...
def cross_validation(data: _GraphDataset,
    ...
    for hyp_idx, hyperparams_sett in enumerate(hyperparams_list):
        ...
        complete_cv_results[hyp_idx]['params'] = hyperparams_sett
        simple_cv_results.loc[hyp_idx, 'params'] = str(hyperparams_sett)

    # ----- cycle across cv rounds
    for round_idx in range(cv_rounds):
        print(f"running cv_round {round_idx+1}/{cv_rounds}")

        # ----- retrieve data splits
        perm_idxs = np.random.default_rng(round_idx).permutation(num_samples)
        tr_idxs, vl_idxs, ts_idxs = perm_idxs[:tr_end_idx], perm_idxs[tr_end_idx:vl_end_idx], perm_idxs[vl_end_idx:]
        ...
        for hyp_idx, hyperparams_sett in enumerate(tqdm(hyperparams_list)):
            ...
            # ----- evaluate scorers and update cv average/std
            for score_name, scorer in evaluation_scorers.items():
                for split_name, split_dict in zip(('train', 'validation', 'test'),
                                                  (tr_dict, vl_dict, ts_dict)):
                    split_res = scorer(split_dict)
                    complete_cv_results[hyp_idx][f'complete_{split_name}_{score_name}'].append(split_res)
                    complete_cv_results[hyp_idx][f'mean_{split_name}_{score_name}'] = np.nanmean(complete_cv_results[hyp_idx][f'complete_{split_name}_{score_name}'])
                    complete_cv_results[hyp_idx][f'std_{split_name}_{score_name}'] = np.nanstd(complete_cv_results[hyp_idx][f'complete_{split_name}_{score_name}'])
                    simple_cv_results.loc[hyp_idx, f'mean_{split_name}_{score_name}'] = complete_cv_results[hyp_idx][f'mean_{split_name}_{score_name}']
                    simple_cv_results.loc[hyp_idx, f'std_{split_name}_{score_name}'] = complete_cv_results[hyp_idx][f'std_{split_name}_{score_name}']
        ...
        pickle.dump([dict(def_dict) for def_dict in complete_cv_results],
                    open(os.path.join(file_path, f'{data}_complete.pickle'), 'wb'))
